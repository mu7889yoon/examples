    ...
    <main className="page-shell">
      ...
      {screen === 'arena' && session && (
        <section className="arena-layout">
          ...
          {laughPulse > 0 && <><div className="laugh-bars" key={laughPulse} style={{ '--laugh-depth': laughDepth } as React.CSSProperties} aria-hidden="true"><span className="laugh-bar top" /><span className="laugh-bar right" /><span className="laugh-bar bottom" /><span className="laugh-bar left" /></div><button type="button" className="laugh-dismiss" onClick={dismissIppon} aria-label="黄色い枠の演出を閉じる" title="演出を閉じる">×</button></>}
